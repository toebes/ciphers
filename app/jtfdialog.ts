 ...
 */
export function JTFDialog(id: string, title: string, content: JQuery<HTMLElement>,
                          okid: string, oktitle: string): JQuery<HTMLElement> {
    // The dialog has a top bar with a title indicating the purpose of the dialog
    // The data-reveal class keeps it hidden
    let dlg = $("<div/>", { class: "reveal", id: id, 'data-reveal': '' })
        .append($("<div/>", { class: "top-bar Primary" })
    ...
    dlg.append($("<button/>", { class: "close-button", "data-close": "", "aria-label": "Close reveal", type: "button" })
