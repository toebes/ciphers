...
export class CipherRunningKeyEncoder extends CipherVigenereEncoder {
    ...
    genPreCommands(): JQuery<HTMLElement> {
        ...
        $("<span/>", { class: "input-group-label" }).text("title")
            .appendTo(inputgroup)
        let select = $("<select/>", { id: "runningkey", class: "lang input-group-field" })
        select.append($('<option />', { value: "" }).text("--Select a Running Key--"))
        for (let entry in runningKeys) {
            select.append($("<option />", { value: entry })
                .text(runningKeys[entry].title + " - " + runningKeys[entry].text.substr(0, 50) + "..."))
        }
    ...
    attachHandlers(): void {
        ...
        $('#runningkey').off('change').on('change', (e) => {
