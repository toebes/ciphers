...
export class CipherCryptarithmEncoder extends CipherEncoder {
    ...
    public updateOutput(): void {
        ...
        this.guidanceURL = 'TestGuidance.html#Cryptarithm' + this.state.operation;
    ...
    public placeCrib(): ICribPos[] {
        ...
        if (this.state.solclick1 !== -1 && this.state.solclick2 !== -1) {
            ...
            const ct1 = this.Cryptarithmchar(pt1);
            const ct2 = this.Cryptarithmchar(pt2);
    ...
    public genSampleQuestionText(): string {
        let msg = '';
        if (this.state.operation === 'crypt') {
            msg = '<p>The following quote has been encoded using the Cryptarithm Cipher. ';
            const cribpos = this.placeCrib();
            const ptstring = this.minimizeString(this.state.cipherString);
            if (cribpos === undefined) {
            ...
            if (this.state.operation === 'encode') {
                msg =
                    ...
                    ' with the Cryptarithm Cipher using ';
            } else {
                msg =
                    '<p>The following quote needs to be decoded ' +
                    ' with the Cryptarithm Cipher where ';
            }
            msg +=
                '<strong><i>a</i>=' +
    ...
    public Cryptarithmchar(c: string): string {
        const charset = this.getCharset();
    ...
    public buildReplacement(msg: string, maxEncodeWidth: number): string[][] {
        ...
        for (let i = 0; i < msgLength; i++) {
            ...
            if (this.isValidChar(messageChar)) {
                message += messageChar;
                cipherChar = this.Cryptarithmchar(messageChar);
                cipher += cipherChar;
            } else {
                message += messageChar;
                cipher += messageChar;
    ...
    public canSolve(m1: string, m2: string): boolean {
        const charset = this.getCharset();
        const c1 = this.Cryptarithmchar(m1);
        const c2 = this.Cryptarithmchar(m2);
    ...
    public genEncodeSolution(): JQuery<HTMLElement> {
        ...
        for (const m of msg) {
            const c = this.Cryptarithmchar(m);
